...
const ENTITY_FILENAME_PATTERN =
  /^(?:0|[1-9][0-9]*)_(?:[A-Z][a-zA-Z]*|[1-9][0-9]*)(?:-(?:[a-zA-Z]+|[1-9][0-9]*))*\.yml$/
...
export const createFileNameValidator =
  (pattern: RegExp): FileNameValidator =>
  (filePath: string) => {
    const fileName = basename(filePath)
    return pattern.test(fileName) ? ok(undefined) : error([fileNameError(pattern, fileName)])
  }

export const validateEntityFileName: FileNameValidator =
  createFileNameValidator(ENTITY_FILENAME_PATTERN)

export const validateLanguageFileName: FileNameValidator =
  createFileNameValidator(LANGUAGE_FILENAME_PATTERN)
