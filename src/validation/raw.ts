import { Result, isOk } from "@optolith/helpers/result"
...
export type TypeValidationResult<K extends keyof TypeMap> = Result<
  TypeIdPair<K>,
  TypeValidationError[]
>

export type TypeValidationResultsByType = [
  keyof TypeMap,
  [filePath: string, result: TypeValidationResult<keyof TypeMap>][]
][]
...
const getFiles = async (path: string): Promise<[path: string, data: unknown][]> =>
  (await readdir(path))
    ...
    .map(
      mapToSecondAsync(async filePath => {
        try {
    ...
    mapSecond((res, path) =>
      Result.reduce(
        ...
        ([id, data]): TypeValidationResult<K> =>
          Result.map(typeConfig.integrityValidator(validators, data, path), () => [id, data]),
...
export const getRawValidationResults = async (
  ...
  const entityDirectoryPathEntries = Object.entries(entityDirPaths) as [
    typeName: keyof TypeMap,
    path: string
  ][]

  const structuralIntegrityResults = await Promise.all(
    entityDirectoryPathEntries.map(
      mapSecondAsync(async (path, typeName) => {
        const files = await getFiles(path)

        return validateStructuralIntegrityOfFiles(
          validator,
          configMap[typeName] as TypeConfig<
            TypeMap[keyof TypeMap],
            TypeId<keyof TypeMap>,
            TypeName<keyof TypeMap>
          >,
  ...
  if (checkIntegrity) {
    const identityIntegrityValidators = Object.fromEntries(
      structuralIntegrityResults.map(
        mapSecond(results => {
          const idCount = results.reduce(
            (mp, [_, result]) =>
              isOk(result) ? mp.set(result.value[0], (mp.get(result.value[0]) ?? 0) + 1) : mp,
    ...
    return structuralIntegrityResults.map(
      mapSecond((results, typeName) =>
